...
主にインフラ周辺を構築する際に実感しました。DockerやAWS EC2, Nginxなどについては、知識不足からとても足踏みしていました。しかし、どの技術を導入する際にもエラーが生じる場合には、エラーコードが発生している為、そのコードと向き合う事で前に進むことが出来ました。
これからまずコードを書き、そのエラーコードから修正方法を学ぶ、という姿勢を大切に開発に取り組みたいと思います。
...
CircleCIを用いて、AWS EC2で構成される本番環境に対し、継続的デプロイを導入した。
